...
class OrderBook:
    ...
    def add_order(self, order: Order):
        ...
        logger.info(f"Added {order.symbol} {order.side.value} {order.order_type.value} order {order.order_id} at {order.price} for {order.remaining_quantity}")
...
class MatchingEngine:
    ...
    def submit_order(self, symbol: str, order_type: str, side: str,
        ...
        try:
            ...
            logger.info(f"Received {symbol} {side} {order_type} order {order.order_id} at {price_decimal} for {str(qty)}")
    ...
    def _add_conditional_order(self, order: Order) -> Dict:
        ...
        logger.info(f"Accepted conditional {order.symbol} {order.side.value} {order.order_type.value} order {order.order_id}, waiting for trigger at {order.price}.")
        return {
            'status': order.status.value,
            'order_id': order.order_id,
